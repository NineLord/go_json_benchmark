...
type MathDataCollectors struct {
	averageGeneratingJsons           MathDataCollector.MathDataCollector
	averageIteratingJsonsIteratively MathDataCollector.MathDataCollector
	averageIteratingJsonsRecursively MathDataCollector.MathDataCollector
	averageDeserializingJsons        MathDataCollector.MathDataCollector
	averageSerializingJsons          MathDataCollector.MathDataCollector
	totalAverageCpu                  MathDataCollector.MathDataCollector
	totalAverageRam                  MathDataCollector.MathDataCollector
}

type aboutInformation struct {
	jsonPath         string
	sampleInterval   uint
...
type ExcelGenerator struct {
	...
	mathDataCollectors MathDataCollectors
}

func MakeExcelGenerator(jsonPath string, sampleInterval, numberOfLetters, depth, numberOfChildren uint) (ExcelGenerator, error) {
	...
	return ExcelGenerator{
		...
		mathDataCollectors: MathDataCollectors{
			averageGeneratingJsons:           MathDataCollector.MakeMathDataCollector(),
			averageIteratingJsonsIteratively: MathDataCollector.MakeMathDataCollector(),
			averageIteratingJsonsRecursively: MathDataCollector.MakeMathDataCollector(),
			averageDeserializingJsons:        MathDataCollector.MakeMathDataCollector(),
			averageSerializingJsons:          MathDataCollector.MakeMathDataCollector(),
			totalAverageCpu:                  MathDataCollector.MakeMathDataCollector(),
			totalAverageRam:                  MathDataCollector.MakeMathDataCollector(),
		},
	}, nil
}

func NewExcelGenerator(jsonPath string, sampleInterval, numberOfLetters, depth, numberOfChildren uint) (*ExcelGenerator, error) {
	if result, err := MakeExcelGenerator(jsonPath, sampleInterval, numberOfLetters, depth, numberOfChildren); err != nil {
		return nil, err
	} else {
		return &result, nil
	}
